...
| **仮想コントローラ** | `自動` は実機パッドが見えていない間だけ表示します。`OFF` / `ON` は固定。なお、ブラウザは実機パッドを「一度使われるまで」見つけないので、`自動` では起動直後だけ表示され、実機パッドが認識された時点で消えることがあります。 |
| **タップしたとき** | `続きから` / `最初から`。 |
...
⚙ →**Python パッケージ**→**モジュールを管理**で、そのゲーム専用の画面が開きます。
1行に1モジュールです。

- **読み込む** — このゲームが読み込むもの。「外す」で一覧から外します。
- **不足しているもの** —「調べる」でゲームのソースを読み、インタプリタに
  「どれが見つからないか」を問い合わせ、1つずつ「取得」ボタンを並べます。
  取得すると、そのゲームで読み込む設定にもなります。なお、これはソースから
  読んだ名前なので、ゲームが「あれば使う」だけの import も混じります。その中には
  PyPI 上の無関係なプロジェクトと名前が同じものもあるので、**エラーに出た名前**を
  取得してください。一覧すべてを取る必要はありません。
- **この版で導入済み** — この版向けに端末にはあるが、このゲームでは
  読み込んでいないもの。「追加」で、再ダウンロードせずに有効化できます。

すでに止まったゲームなら、表示されるエラー画面から同じ画面が開き、そのまま
調べます。

導入できるのは**そのインタプリタ向けにビルドされた wheel** だけで、入手元は
Pyodide の配布物か PyPI です。そのビルドが存在しないものは導入できません。
これはこちらではどうにもなりません。

パッケージは**ゲームの Pyxel バージョンに追従**します。古い Pyxel は古い
Python で動くため、片方向けの wheel はもう片方では読み込めません。そこで版ごとに
別々に持ちます。管理画面には対象の Pyxel・Python・ABI が表示され、その版の分が
まだ無いモジュールには印が付きます。もう一度調べれば、その版に合うものを取得します。

取得したものはランタイム設定に一覧され、ビルド対象の Python（`[cp314]` など）が
併記されます。そこから削除できます。
